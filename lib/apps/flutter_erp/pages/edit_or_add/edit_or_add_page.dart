...
class EditOrAddPage extends StatefulWidget {
  ...
  const EditOrAddPage({Key? key, required this.addMode, this.contact})
...
class _EditOrAddPageState extends State<EditOrAddPage>
  ...
  void didChangeDependencies() {
    final bloc = context.read<EditOrAddBloc>();

    _subscriptionIsLoading ??= bloc.isLoading$.listen((isLoading) {
      if (isLoading) {
    ...
    _subscriptionMessage ??= bloc.message$.listen((message) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        title: Text(widget.addMode ? 'Add contact' : 'Edit contact'),
      ),
      body: Container(
              ...
              children: <Widget>[
                  ...
                  child: StreamBuilder<Gender>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return RadioGroup<Gender>(
                        groupValue: gender,
                        onChanged: (value) {
                          bloc.genderChanged(value);
                        },
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: const <Widget>[
                            Radio<Gender>(value: Gender.male),
                            Text('Male', style: TextStyle(fontSize: 16.0)),
                            Radio<Gender>(value: Gender.female),
                            Text('Female', style: TextStyle(fontSize: 16.0)),
                          ],
                        ),
                      );
                    },
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    child: Text(widget.addMode ? 'Add' : 'Update'),
                    /*padding: const EdgeInsets.all(24),
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(8),
                    ),
                    color: Theme.of(context).cardColor,
                    splashColor: Colors.blue,//Theme.of(context).accentColor,*/
