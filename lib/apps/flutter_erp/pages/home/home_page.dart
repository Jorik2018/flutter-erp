...
import 'package:flutter_erp/apps/flutter_erp/domain/contact_repository.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  void didChangeDependencies() {
    ...
    _subscription ??= bloc.message$.listen(_handleMessage);
  }

  @override
  Widget build(BuildContext context) {
    final bloc = context.read<HomeBloc>();
    final listPadding = MediaQuery.of(
      context,
    ).padding.copyWith(left: 0.0, right: 0.0, top: 0.0, bottom: 56 + 16);
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) {
                return Provider<EditOrAddBloc>(
                  create: (providerContext) {
                    return EditOrAddBloc(
                      providerContext.read<ContactRepository>(),
                      true,
                    );
                  },
                  dispose: (_, bloc) => bloc.dispose(),
                  child: const EditOrAddPage(addMode: true),
                );
              },
            ),
          );
        ...
        children: <Widget>[
          ...
          Expanded(
            child: StreamBuilder<HomeState>(
              stream: bloc.state$,
              initialData: bloc.state$.valueOrNull,
...
class _HomeAppBarState extends State<HomeAppBar> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _opacityTextField = Tween<double>(
      begin: 0,
      end: 1,
    ).animate(CurvedAnimation(parent: _controller, curve: Curves.easeIn));
    _opacityTitle = Tween<double>(
      begin: 1,
      end: 0,
  ...
  @override
  Widget build(BuildContext context) {
    final bloc = context.read()<HomeBloc>(context);
    ...
    return Container(
      child: AppBar(
        ...
        title: _isSearching
            ...
            : FadeTransition(opacity: _opacityTitle, child: Text('Home page')),
        actions: <Widget>[
          IconButton(
            tooltip: 'Delete all',
