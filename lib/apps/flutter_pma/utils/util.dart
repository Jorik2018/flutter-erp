  ...
  Future<http.Response?> get(String url, {var headers, State? state}) async {
    ...
    try {
      response = await http.get(
        Uri.parse(url.startsWith("http") ? url : (Util.API_URL + url)),
        headers: h,
      );
    ...
    return DropdownButton(
      ...
      items: _options.map((_o) {
        if (adapter != null) {
          ...
          return new DropdownMenuItem(value: l[0], child: Text(l[1]));
        } else
          return new DropdownMenuItem(value: _o, child: Text(_o));
      }).toList(),
      onChanged: (e) {
        setState(() {
          _o[key] = e;
