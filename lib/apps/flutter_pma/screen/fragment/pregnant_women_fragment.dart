...
class PregnantWomenFragment extends StatefulWidget {
  LX? mm;

  PregnantWomenFragment({this.mm});

  @override
  _PregnantWomenFragmentState createState() => _PregnantWomenFragmentState();
}

class _PregnantWomenFragmentState extends State<PregnantWomenFragment> {
  int page = 0;
  int limit = 50;

  Future _getItems(page, limit) async {
    http.Response? response = await http2.get(
      '/api/minsa/pregnant-woman/' + page.toString() + '/' + limit.toString(),
      state: this,
    );
    try {
  ...
  void initState() {
    ...
    widget.mm!.observer!('title', 'Gestantes');
  }
              ...
              child: GestureDetector(
                onTap: () {
                  for (int i = 0; i < _selected.length; i++) {
                    if (_selected[i] == true) {
                      context.go(
                        '/pregnant-woman/' + _data[i]['_id']['\$oid'] + '/edit',
                      );
                      break;
                    }
                  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          context.push('/pregnant-woman/create');
        },
        ...
        children: [
          ...
          Expanded(
            child: DataTable2(
              ...
              minWidth: 600,
              columns: [
                DataColumn2(label: Text('DNI'), size: ColumnSize.M),
                DataColumn2(
                  label: Text('Nombre Completo'),
                  size: ColumnSize.L,
                  fixedWidth: 200,
                ),
                DataColumn(label: Text('Fecha nacimiento / Edad')),
                DataColumn2(label: Text('Distrito'), size: ColumnSize.L),
