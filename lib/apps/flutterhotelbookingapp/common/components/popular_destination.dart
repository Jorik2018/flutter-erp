...
import 'package:flutter_erp/apps/flutterhotelbookingapp/common/components/Section.dart';
import 'package:flutter_erp/apps/flutterhotelbookingapp/common/components/circular_progress.dart';
import 'package:flutter_erp/apps/flutterhotelbookingapp/model/destination.dart';
import 'package:flutter_erp/apps/flutterhotelbookingapp/screen/place_detail_screen.dart';
import 'package:flutter_erp/apps/flutterhotelbookingapp/utils/Urls.dart';
      ...
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            SizedBox(width: 10),
            Text("Popular Destinations", style: kHeadingextStyle),
          ],
        ),
        DestinationList(),
      ],
    ...
    destinations.forEach((destination) {
      Widget element = Container(
        ...
        child: GestureDetector(
          child: PopularDestinationCard(destination: destination),
          onTap: () {},
        ),
      );
      list.add(element);
...
class PopularDestinationCard extends StatefulWidget {
  ...
  PopularDestinationCard({required this.destination});
...
Future<List<Destination>> getDestinationList() async {
  if (destinations == null) {
    Response response;
    response = await get(Uri.parse(Urls.POPULAR_DESTINATION));
    int statusCode = response.statusCode;
    final body = json.decode(response.body);
    if (statusCode == 200) {
      destinations = (body as List)
