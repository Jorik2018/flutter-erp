...
import 'package:flutter_erp/apps/flutterhotelbookingapp/model/hotel_model.dart';
    ...
    return Column(
      children: <Widget>[
        Row(
          children: <Widget>[
            Image.asset(
              "assets/images/icons/ic_room.png",
              width: 20,
              height: 20,
              color: Color(0xFF3443fa),
            ),
            SizedBox(width: 10),
            Text("Featured for you", style: kHeadingextStyle),
          ],
        ),
...
Widget createListView(BuildContext context, AsyncSnapshot snapshot) {
  ...
  return GridView.count(
    ...
    children: List<Widget>.generate(hotels.length, (index) {
      return GridTile(
        child: InkResponse(
          child: HotelItemCard(hotel: hotels[index]),
          onTap: () {},
        ),
      );
    }),
...
class HotelItemCard extends StatefulWidget {
  ...
  HotelItemCard({required this.hotel});
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: Stack(
          children: <Widget>[
            Positioned.fill(
              child: Image.network(widget.hotel!.thumbnail!, fit: BoxFit.cover),
            ),
            Positioned(
              bottom: 0,
              left: 0,
...
Future<HotelModel> getHotelList(String pageNumber) async {
  if (hotelModel == null) {
    Response response;
    response = await get(Uri.parse(Urls.HOTEL_SAMPLE_LIST + pageNumber));
