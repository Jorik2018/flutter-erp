...
import 'package:flutter_erp/apps/wonders/ui/utils/random_utils.dart';
...
class _AnimatedCloudsState extends State<AnimatedClouds> with SingleTickerProviderStateMixin {
  ...
  List<_Cloud> _getClouds() {
    Size size = ContextUtils.getSize(context) ?? Size(context.size!.width, 400);
    int rndSeed = _getCloudSeed(widget.wonderType);
...
class _Cloud extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Transform.scale(
        ...
        child: Image.asset(
          ...
          width: context.size!.width * .65 * scale,
          fit: BoxFit.fitWidth,
        ),
      );
}
