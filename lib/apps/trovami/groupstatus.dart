...
final userref = FirebaseDatabase.instance.ref().child('users');          // new
final groupref = FirebaseDatabase.instance.ref().child('groups');          // new
  ...
  grps.forEach((k,v) async{
    ...
    if(v["groupname"]==groupStatusGroupname) {
      for(var i=0;i<v["members"].length;i++) {
        var response1 = await http.get(Uri.parse("https://trovami-bcd81.firebaseio.com/groups/${k}/members/${i}.json"));
//        print(response1);
        ...
        for(var i=0;i<v["members"].length;i++) {
          ...
          if(result1["emailid"]==loggedinUser){
            if(result1["locationShare"]==true||result1["location"]==null) {
              await httpClient.put(
                  url: Uri.parse("https://trovami-bcd81.firebaseio.com/groups/${k}/members/${i}/location.json"),body: locUpdated);
            }
