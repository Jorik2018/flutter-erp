...
import 'package:flutter_erp/apps/fluter_uber_clone/models/placePrediction.dart';
...
class _SearchPageState extends State<SearchPage> {
  ...
  void findPlace(String placeName) async {
    if (placeName.length > 1) {
      ...
      var res = await HttpClient.get(autoCompleteUrl);
      if (res == "failed") {
        return;
      }
      if (res.success) {
        var prediction = res.body;
...
class PredictionTile extends StatelessWidget {
  ...
  void getAddressDetails(String placeId, context) async {
    ...
    var res = await HttpClient.get(placeAddressUrl);
    Navigator.pop(context);
    if (res == "failed") {
      return;
    }
    ...
    if (res.success == "OK") {
      ...
      address.placeName = data["result"]["name"];
