...
class DesktopLoginForm extends StatefulWidget {
  const DesktopLoginForm({required this.authRepository, Key? key})
    : super(key: key);
...
class _DesktopLoginFormState extends State<DesktopLoginForm> {
  ...
  void initState() {
    super.initState();
    _loginBloc = LoginBloc(authRepository: widget.authRepository);
    _emailTextController.addListener(
      () => setState(() => _email = _emailTextController.text),
    );
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [BlocProvider(create: (context) => _loginBloc)],
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: onClick,
        child: Image.asset(socialIconAsset, width: 21, height: 21),
      ),
    );
  }
}
