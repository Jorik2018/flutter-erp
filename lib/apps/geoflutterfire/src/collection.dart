...
import 'dart:async';
import 'package:flutter_erp/apps/geoflutterfire/src/models/DistanceDocSnapshot.dart';
...
class GeoFireCollectionRef {
  ...
  Future<void> setDoc(String id, var data, {bool merge = false}) {
    /**erro:The named parameter 'merge' isn't defined.
Try correcting the name to an existing named parameter's name, or defining a named parameter with the name 'merge' */
  ...
  Future<void> setPoint(
      ...
      return _collectionReference!.doc(id).set({'$field': point}, SetOptions(merge: true));
  }
  ...
  Stream<List<DocumentSnapshot>> within(
    ...
    Iterable<Stream<List<DistanceDocSnapshot>>> queries = area.map((hash) {
      ...
      return _createStream(tempQuery).map((QuerySnapshot querySnapshot) {
        /**error:The argument type 'Null' can't be assigned to the parameter type 'double'. */
...
Try making the call conditional (using '?.') or adding a null check to the target ('!'). */
        var geoPointField = (distanceDocSnapshot.documentSnapshot.data() as Map)[fieldList[0]];
