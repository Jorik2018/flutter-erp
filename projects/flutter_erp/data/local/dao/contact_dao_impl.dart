import 'package:rxdart/rxdart.dart';
//import 'package:sqflite/sqflite.dart';
//import 'package:sqlbrite/sqlbrite.dart';

import 'contact_dao.dart';
import 'contact_entity.dart';
import 'contact_entry.dart';

class ContactDaoImpl implements ContactDao {
  //final Future<BriteDatabase> _briteDatabaseFuture;

  //const ContactDaoImpl(this._briteDatabaseFuture);
  //const 
  ContactDaoImpl();

  List<ContactEntity> contacts = [
    //ContactEntity(id: 1, name: 'Alice', address: '123 Main St', phone: '555-1234'),
    //ContactEntity(id: 2, name: 'Bob', address: '456 Oak St', phone: '555-5678'),
    // Add other sample contacts as needed
  ];

  @override
  Stream<List<ContactEntity>> search(String query) {
    /*return Stream.fromFuture(_briteDatabaseFuture).flatMap((db) {
      return db
          .createQuery(
            tableContacts,
            where:
                '$columnName LIKE ? OR $columnAddress LIKE ? OR $columnPhone LIKE ?',
            whereArgs: [
              '%$query%',
              '%$query%',
              '%$query%',
            ],
            orderBy: '$columnName ASC',
          )
          .mapToList((row) => ContactEntity.fromJson(row));
    });*/
        List<ContactEntity> searchResults = contacts
        .where((contact) =>
            contact.name.contains(query) ||
            contact.address.contains(query) ||
            contact.phone.contains(query))
        .toList();

    return Stream.value(searchResults);
  }

  @override
  Stream<ContactEntity?> findById(int id) {
    /*return Stream.fromFuture(_briteDatabaseFuture).flatMap((db) {
      return db
          .createQuery(
            tableContacts,
            where: '$columnId = ?',
            whereArgs: [id],
            limit: 1,
          )
          .mapToOneOrDefault((row) => ContactEntity.fromJson(row), null);
    });*/
        // Simulate finding a contact by its ID
    ContactEntity? foundContact =
        contacts.firstWhere((contact) => contact.id == id);

    return Stream.value(foundContact);
  }

  @override
  Future<bool> deleteById(int id) async {
    /*final db = await _briteDatabaseFuture;
    final rows = await db.delete(
      tableContacts,
      where: '$columnId = ?',
      whereArgs: [id],
    );*/
    final rows = 1;
    return rows > 0;
  }

  @override
  Future<bool> update(ContactEntity entity) async {
    /*final db = await _briteDatabaseFuture;
    final rows = await db.update(
      tableContacts,
      entity.toJson(),
      where: '$columnId = ?',
      whereArgs: [entity.id],
      conflictAlgorithm: ConflictAlgorithm.replace,
    );
    return rows > 0;**/
    return true;
  }

  @override
  Future<bool> insert(ContactEntity entity) async {
    /*final db = await _briteDatabaseFuture;
    final id = await db.insert(
      tableContacts,
      entity.toJson(),
      conflictAlgorithm: ConflictAlgorithm.replace,
    );
    return id != -1;*/
    return true;
  }

  @override
  Future<void> deleteAll() async {
    /*final db = await _briteDatabaseFuture;
    await db.delete(
      tableContacts,
      where: null,
    );*/
  }
}
