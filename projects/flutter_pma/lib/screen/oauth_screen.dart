...
class OAuthScreen extends StatefulWidget {
  ...
  @override
  _OAuthScreenState createState() => new _OAuthScreenState();
...
class _OAuthScreenState extends State<OAuthScreen> {
  ...
  void _initToken(String code) {
    ...
    http2.post('/api/auth/token',code,headers: (h) {
      h.clear();
      return h;
    }).then((response) {
      ...
      if (result['error'] == null)
        Hive.openBox('app').then((boxApp) {
          boxApp.put('token', result['access_token']??result['token']).then((value) {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      body: new Container(
          padding: new EdgeInsets.all(20.0), child: Text('OAuth->' + msg)),
    );
  }
