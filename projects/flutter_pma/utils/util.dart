...
class Util {
  ...
  static String API_URL = String.fromEnvironment('API_URL');
...
class HTTP {
  ...
  Future<Response?> get(String url, {var headers, State? state}) async {
    ...
    try {
      response = await _dio.get(
        url.startsWith("http") ? url : (Util.API_URL + url),
  ...
  Future post(
    ...
    State? state,
    void Function(String, Response?)? onError,
    var headers,
  }) async {
    ...
    if (response.statusCode == 500 ||
      ...
      if (onError != null)
        ...
        Alert(state.context, '', title: 'Error');
    }
    return response;
  }
  ...
  Future gql(
    ...
    try {
      ...
      var v = response.data is String ? jsonDecode(response.data) : response.data;
      if (v['errors'] != null) {
        if (onError != null)
...
class FormBuilder {
    ...
    return DropdownButton(
      ...
      items: _options.map((_o) {
        if (adapter != null) {
          ...
          return DropdownMenuItem(value: l[0], child: Text(l[1]));
        } else
          return DropdownMenuItem(value: _o, child: Text(_o));
      }).toList(),
      onChanged: (e) {
        setState(() {
          _o[key] = e;
  ...
  Widget textField(
    ...
    bool readOnly = false,
    Function()? onTap,
    String Function(dynamic v)? adapter,
  }) {
    if (_setState != null && setState == null) setState = _setState;
    TextEditingController? controller = controllerMap[name];
    if (controller == null) {
    ...
    return (panels.map<ExpansionPanel>((e) {
      ...
      return ExpansionPanel(
        headerBuilder: (BuildContext context, bool isExpanded) {
          return ListTile(title: Text(e['title'], style: bold20Style));
        },
